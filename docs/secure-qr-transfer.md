...
repository source of truth, verify and check out the intended commit, copy its
`HEAD` runner into dom0, fetch with `--repo-vm <VM> --fetch-only`, review the fetched tree,
stage with `--stage-only`, and build

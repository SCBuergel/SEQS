...
whenever Qubes rebuilds its rules. A separate output-hook rule translates only
DNS requests generated by `A-wireguard` itself for those same Qubes addresses;
this keeps name resolution working inside the provider without rewriting its
resolver file. The VPN qube itself can still contact its upstream network so it

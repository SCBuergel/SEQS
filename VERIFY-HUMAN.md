...
- **Re-running `setup-qubes.sh`**: every build again requires `--qubes ...` or explicit `--all`; the previous selection is audit history, never an implicit default. Re-runs **converge** — selected SEQS-built qubes are reported and reconfigured in place, finished components are skipped via their `/rw/config/seqs` markers, unselected managed qubes are untouched, and qubes SEQS did not build are refused. `--postfix NAME` applies one validated postfix to every selected recipe and creates or converges a separate managed instance; confirm the root-owned selection and target files retain the recipe-to-instance mapping. For GnosisVPN, confirm every temporary `qubes.UpdatesProxy` allow/deny pair names an exact selected template instance and that the policy and proxy are removed afterward. Use `./delete-vms.sh <instance-name>` only when you want to rebuild an instance from scratch. The policy-takeover `[y/N]` prompt only fires for policy files SEQS does not own — if it fires on a re-run, read the dump.

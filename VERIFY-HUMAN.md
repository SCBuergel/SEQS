...
1. **Fetch and stage.** The integrity anchor is the **git commit hash** you verified in the disposable (`git rev-parse HEAD` against the value you publish/trust) — it covers the whole repository, including `setup-qubes.sh`. Fetch saves a validated tree under `/var/lib/seqs/fetched` and prints a `Transfer SHA256` of the tar stream, which is a diagnostic only (it confirms faithful transport from the source qube, not that the qube is honest). Review the tree before `--stage-only`; staging shows the diff that will be placed under `/srv`, and re-running `--stage-only` reports whether the staged tree is identical to the fetched one.

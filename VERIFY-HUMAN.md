...
- If QR webcam mode is enabled, `qvm-pci list --assignments` must show the reviewed controller assigned only to `sys-usb-webcam` in dedicated mode, or to both `sys-usb` and `sys-usb-webcam` in sequential mode. `qvm-pci list sys-usb` must show the selected controller before a sequential build. Do not use `qvm-prefs ... pcidevs`; PCI ownership is exposed through `qvm-pci`, not a VM preference.
- Before using QR transfer with a real secret, rehearse with disposable test data and confirm the trusted source and target independently display the same four groups of five ciphertext-fingerprint characters before GnuPG runs. The passphrase must be the only value written on paper; the expected fingerprint must not travel through the QR channel.
...
- **Re-running `setup-qubes.sh`**: every build again requires `--qubes ...` or explicit `--all`; the previous selection is audit history, never an implicit default. Re-runs generally **converge** — selected SEQS-built qubes are reported and reconfigured in place, finished components are skipped via their `/rw/config/seqs` markers, unselected managed qubes are untouched, and qubes SEQS did not build are refused. GnosisVPN is intentionally stricter: if either selected `Z-gnosisvpn[-POSTFIX]` or `A-gnosisvpn[-POSTFIX]` already exists, the runner must fail before applying Salt or creating the temporary proxy. `--postfix NAME` applies one validated postfix to every selected recipe and creates a separate managed instance. For a fresh GnosisVPN build, confirm every temporary `qubes.UpdatesProxy` allow/deny pair names its exact selected template instance and that the policy and proxy are removed afterward. Use `./delete-vms.sh <instance-name>` only when you intentionally want to rebuild an instance from scratch. The policy-takeover `[y/N]` prompt only fires for policy files SEQS does not own — if it fires on a re-run, read the dump.

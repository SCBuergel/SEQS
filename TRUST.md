...
- **Residual risk:** The fetched tree becomes root-executed Salt code in dom0, so a compromised `REPO_VM` = compromised dom0 **unless the operator actually reviews at the gate** (diff + `CONTINUE` prompt, or `--fetch-only` audit). A dedicated, minimal qube is preferable to a daily-driver.

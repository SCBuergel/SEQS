...
State as of **2026-07-05**, reflecting the migration of the installer from an imperative dom0 bash script to the Qubes Salt management stack (`setup-qubes.sh` is now a thin runner; the states live in `salt/seqs/`, the configuration in `salt/pillar/seqs/config.sls`). (Per-component key-fingerprint verification dates remain as captured inside each entry; this top-line date tracks the document itself.)
...
- **Guidance for the operator:** The README's first-install path uses a fresh networked DisposableVM, explicitly passes its name through `SEQS_REPO_VM`, keeps it alive only through `--fetch-only`, and then destroys it. Do **not** use a daily `personal` qube. For ongoing maintenance, either repeat the disposable workflow or use a dedicated, minimal, network-light repo qube. `setup-qubes.sh` retains `personal` only as a legacy compatibility fallback; the documented path does not rely on it.
...
- **Residual risk:** The gate is only as good as the operator's diligence at it. On the first fetch there is no previous tree to diff against, so "review" means `--fetch-only` + reading `/srv`, or trusting the SHA256 comparison. This replaces the old installer's continuous per-file fetching from the live repo qube (scripts, libs, assets and *directory listings*, interpolated into remote shell commands) — the VM→dom0 dataflow is now one reviewable artifact.
...
> **Interrupted or failed runs.** The old installer's per-qube watchdog + rollback is gone; the flow is **convergent** instead. A failed or interrupted run is re-run as-is: completed components are skipped via `/rw/config/seqs/` markers, existing qubes are reconfigured, and a qube created-but-not-yet-tagged by an interrupted run is adopted on the next run via its intent marker in `/var/lib/seqs/intents/` (a same-named qube *without* marker or `seqs-managed` feature is refused — see no-clobber below). Per-component `timeout:` on the salt `cmd.run` states (pillar `component_timeout`, default 900 s) bounds a hung installer; salt kills only the command it spawned, so after a timeout inside a template treat that template's state as suspect and re-run — the component re-executes from scratch because its completion marker was never written.
...
- **Established by:** 📝 Reviewed; shipped in the same validated tar transfer, delivered via the salt fileserver. The libs are overlaid **after** the component's own files, so a component asset can never shadow `verify-gpg.sh`/`brave.sh` (the old flow aborted on that collision; here the lib simply wins). The `. "$(dirname "$0")/lib.sh"` pattern in the scripts keeps working verbatim.
...
- **Established by:** The QubesOS project — this is the distribution's own management mechanism, the same one `qubesctl state.highstate` users rely on. Trusting it is strictly narrower than the old model, where dom0 itself parsed VM-produced directory listings and piped VM output through its terminal during every build.
...
- **Established by:** ✅ Validated twice — in the `seqs.dom0` pre-flight (nothing is built on failure) and again at render time inside each qube by `seqs.qube`: mode must be `folder` or `contents`, the path must be absolute, strictly under `/home/user/` with at least one extra non-empty segment, contain no `..` component, and use only `[A-Za-z0-9._/-]` characters. So entries like `/home/user`, `/home/user/`, `/etc`, `/`, or `/home/user/../etc` are rejected before any template is built, bounding the destructive blast radius to the qube user's home tree. The charset restriction (no spaces or shell metacharacters, a deliberate narrowing vs the old `printf %q` escaping) means the quoted interpolation into the generated script cannot break out of the cleanup loop.
...
- **Established by:** ✅ Unlike the old installer, **no VM output is executed or parsed in dom0 any more** — per-qube provisioning runs through the disposable management VM, so the sanitizer's role has narrowed from load-bearing input defense to display hardening. It stays because `qubesctl --show-output` embeds strings produced inside target qubes (installer stdout captured by salt). ESC, BEL, CR, FF, SO/SI, the CSI/OSC sequence space and both C1 encodings cannot reach the terminal — they render as the plain-text *letters* of the would-be escape sequence. UTF-8 stays readable.
...
4. **`REPO_VM` + the fetch** (§2) — the repo and its host qube remain dom0-equivalent in effect: the fetched tree is root-executed Salt code. The exposure is now bounded to **one** validated tar transfer behind a diff-and-`CONTINUE` review gate (plus the one-file cat-hack bootstrap of the runner itself, stderr-suppressed), instead of the old continuous per-file fetch-and-interpolate flow — but the gate only helps if the operator actually reads what it shows. The "REPO_VM = dom0 unless you review" envelope is unchanged.
...
The browser-link handoff back-channel that previously undermined every `offline` qube and the wallet qubes is closed by default: the `seqs.dom0` state writes a dom0 deny for `qubes.OpenURL` from every qube spec carrying `offline` or `no_handoff`, and the per-qube xdg default-web-browser is not set for those qubes (so the template-installed handler is never their default). `offline` implies `no_handoff` (enforced both in the pillar slice and at the policy boundary). The default wallet qube specs carry `no_handoff` and the keepass qube carries `offline`.

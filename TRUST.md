...
- **Residual risk:** Runs with the dom0 user's privileges — `qvm-*` management of every qube, plus `sudo` to write the qrexec browser policy. It also fetches and moves install scripts and `lib/*.sh` into VMs. Note also that `setup-qubes.sh` itself does **not** `set -Eeuo pipefail` (every install script under it does). A mid-flight failure inside `installQube` — e.g. `qvm-clone` running out of space, `qvm-run` returning non-zero — does not halt the run; it rolls into the next step and the next qube. Half-built `Z-*`/`A-*` pairs can end up missing the cleanup service, the browser handoff, or an extension force-install while still showing up as "present" in `qvm-ls`, and there is no automatic rollback of a partially-built qube — re-running setup then aborts at `validateAllQubes` because the names already exist, so you must `delete-vms.sh <name>` by hand first. Deferred — current posture is to watch the output and clean up manually on failure.

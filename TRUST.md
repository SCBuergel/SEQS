...
- **Established by:** 📝 Reviewed. The runner requires `--qubes ...` or explicit `--all`, canonicalizes the selection, writes a root-owned runtime intent outside `/srv`, and reports separate staged-tree and build-plan hashes. Salt validates every requested name against the reviewed catalogue; only the selected map drives creation, target generation, provisioning, and air-gap checks, while unselected managed qubes remain untouched. All qube creation and provisioning logic lives in the Salt states; the runner sequences dom0, templates, and app qubes with `qvm-shutdown --wait` barriers so template root volumes are committed before app qubes snapshot them.
